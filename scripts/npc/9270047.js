...
function start() {
	if (cm.getPlayer().getMapId() == 551030200) {
		...
		if (cm.getPlayer().getLevel() < 90) {
			cm.sendOk("您需要到達90 等才可挑戰 Scarlion/Targa.");
			cm.dispose();
			return;
		}
	...
	status = 0;
		...
		cm.sendOk("您已經挑戰過 Scarlion/Targa in the past 3 hours. 剩餘時間: " + cm.getReadableMillis(cm.getCurrentTime(), time + (3 * 360000)));
		cm.dispose();
		return;
	    }
...
function action(mode, type, selection) {
	...
	case 0:
	    if (mode == 1) {
			if (cm.registerSquad("ScarTar", 5, " 您已經成為了遠征隊隊長. 請在時間內請隊員加入.")) {
