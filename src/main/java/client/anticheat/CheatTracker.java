...
public class CheatTracker {
    ...
    private MapleCharacter getPlayer() {
        int ch = World.Find.findChannel(ownerid);
        int wl = World.Find.findWorld(ownerid);
        if (ch > 0) {
            return ChannelServer.getInstance(wl, ch).getPlayerStorage().getCharacterById(ownerid);
        }else
            return null;
    }
...
//                    WorldBroadcastService.getInstance().broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM消息] " + player.getName() + " ID: " + player.getId() + " (等級 " + player.getLevel() + ") 在地圖: " + player.getMapId() + " 打怪時間超過 80 分钟，系统启动测谎仪系统。"));
    ...
    public void checkTakeDamage(int damage) {
        ...
        if (damage == 0) {
            numZeroDamageTaken++;
            if (numZeroDamageTaken >= 100) { // 当次数达到50次就封掉玩家
