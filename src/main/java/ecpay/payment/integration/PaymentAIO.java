...
public class PaymentAIO {
	...
	public String getCustomBill(int amount, String name, int billid, Date date, String ss){
		initial();
		//實體化出所需要的domain物件
		AioCheckOutALL aio = new AioCheckOutALL();
		//合作特店後台自行產生不重複的流水號，或是UUID隨機產生不重複的廠商交易編號(記得把'-'濾掉，僅接受英文字與數字)
		aio.setMerchantTradeNo(ss);
		//合作特店後台填入會員交易時間(不一定是當下，依據需求自行判斷填入)
		String timeStamp = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss").format(date);
		aio.setMerchantTradeDate(timeStamp);
		...
		String cp = aio.getChoosePayment();
