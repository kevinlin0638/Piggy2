...
public abstract class MapleEvent {
            ...
            for (MapleEventType t : MapleEventType.values()) {
                ...
                if (e.isRunning) {
                    for (int i : e.type.mapids) {
                        if (cserv.getEvent() == i) {
                            World.Broadcast.broadcastMessage(worldSt, CWvsContext.broadcastMsg(0, "Entries for the event are now closed!"));
    ...
    public static String scheduleEvent(final MapleEventType event, final ChannelServer cserv) {
        ...
        World.Broadcast.broadcastMessage(worldSt, CWvsContext.broadcastMsg(0, "Hey guys! Let's play a " + StringUtil.makeEnumHumanReadable(event.name()) + " event in channel " + cserv.getChannel() + "! Change to channel " + cserv.getChannel() + " and use @event command!"));
        return "";
    }
