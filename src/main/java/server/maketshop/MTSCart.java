...
public final class MTSCart implements Serializable {
    ...
    public MTSCart(int characterId) throws SQLException {
        ...
        transfer.addAll(ItemLoader.MTS_TRANSFER.loadItems(false, characterId).values().stream().map(Pair::getLeft).collect(Collectors.toList()));
        loadCart();
        loadNotYetSold();
    }
    ...
    public void changeInfo(final int tab, final int type, final int page) {
        if (tab != this.tab || type != this.type) { //isChanged
